 ...
 *
 * @package   KCMS-e-switch
 * @link      https://github.com/KubeeCMS/KCMS-e-switch
 * @author    Kubee
 * @license   GPL v2 or later
 *
 * Plugin Name:  KCMS eSwitch
 * Description:  Instant switching between user accounts.
 * Version:      1.0
 * Plugin URI:   https://github.com/KubeeCMS/KCMS-e-switch.git
 * Author:       weDevs
 * Author URI:   https://github.com/KubeeCMS/KCMS-e-switch/graphs/contributors
...
class employee_switching {
	...
	public function init_hooks() {
		...
		add_filter( 'erp_employee_row_actions', 	   array( $this, 'employee_row_action' ), 10, 2 );
	}
	 ...
	 * @return array
	 */
	public function employee_row_action( $actions, $user_id ) {
		if ( ! current_user_can( 'erp_edit_employee' ) ) {
			return;
		}
		$user = get_userdata( $user_id );
		$link = self::maybe_switch_url( $user );
		$actions['emmployee_switch'] = '<a href="'.$link.'">'.__( 'Switch To', 'user-switching' ).'</a>';
	...
	public static function get_instance() {
		...
		if ( ! isset( $instance ) ) {
			$instance = new employee_switching();
		}
...
if ( ! function_exists( 'user_switching_set_olduser_cookie' ) ) {
	...
	function user_switching_set_olduser_cookie( $old_user_id, $pop = false, $token = '' ) {
		$secure_auth_cookie    = employee_switching::secure_auth_cookie();
		$secure_olduser_cookie = employee_switching::secure_olduser_cookie();
...
if ( ! function_exists( 'user_switching_get_auth_cookie' ) ) {
	...
	function user_switching_get_auth_cookie() {
		if ( employee_switching::secure_auth_cookie() ) {
			$auth_cookie_name = USER_SWITCHING_SECURE_COOKIE;
		} else {
			$auth_cookie_name = USER_SWITCHING_COOKIE;
		}
...
if ( ! function_exists( 'current_user_switched' ) ) {
	...
	function current_user_switched() {
		...
		return employee_switching::get_old_user();
	}
}

$GLOBALS['user_switching'] = employee_switching::get_instance();
